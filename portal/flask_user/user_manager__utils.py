...
class UserManager__Utils(object):
    ...
    def alternate_email_is_available(self, new_email):
        """Check if ``new_email`` is available.

        | Returns True if ``new_email`` does not exist or belongs to the current user's secondary email.
        | Return False otherwise.
        """

        user, user_email = self.db_manager.get_user_and_alternate_email_by_email(new_email)
        return (user == None)
